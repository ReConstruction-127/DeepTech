...
public class InteractionRecipeBuilder implements RecipeBuilder {
	...
	public InteractionRecipeBuilder result(ItemLike item, int weight) {
		results.add(new WeightedResult(new ItemStack(item), weight));
		return this;
	}

	public InteractionRecipeBuilder result(ItemLike item, int count, int weight) {
		results.add(new WeightedResult(new ItemStack(item, count), weight));
		return this;
	}

	public InteractionRecipeBuilder result(ItemStack stack, int weight) {
	...
	public @NotNull Item getResult() {
		return results.isEmpty() ? net.minecraft.world.item.Items.AIR : results.get(0).stack.getItem();
	}

	@Override
	public void save(@NotNull Consumer<FinishedRecipe> consumer, @NotNull ResourceLocation id) {
		if (triggerItem == null) {
			throw new IllegalStateException("Missing trigger item for interaction recipe " + id);
		}
		if (targetState == null) {
			throw new IllegalStateException("Missing target block for interaction recipe " + id);
		}
		if (results.isEmpty()) {
			throw new IllegalStateException("Missing results for interaction recipe " + id);
		}
