...
public class ResonanceNodeClientHelper {
	...
	private static final int PARTICLE_COOLDOWN = 10;
	private static final int PLAYER_CHECK_RANGE = 16;
	private static final int SCAN_INTERVAL = 10;
	private static final int MAX_VISIBLE_CONNECTIONS = 20;
	private static final int MAX_PARTICLES_PER_CONNECTION = 8;
	...
	public void tick(Level level) {
		...
		if (!hasNearbyPlayerWithWrench(level)) {
			return;
		}

		particleTickCounter++;
		if (particleTickCounter % PARTICLE_COOLDOWN != 0) {
			return;
		}
		...
		for (BlockPos targetPos : cachedNodes) {
			if (connectionsShown >= MAX_VISIBLE_CONNECTIONS) {
				break;
			}
